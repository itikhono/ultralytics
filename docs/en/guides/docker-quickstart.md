...
=== "AMD (ROCm)"
    ...
    The `latest-amd` image targets the ROCm 7.2 / MIGraphX wheel stack (`rocm-rel-7.2`), currently validated by the Ultralytics AMD GPU CI. For a different ROCm minor, rebuild `Dockerfile-amd` against AMD's [matching wheel index](https://repo.radeon.com/rocm/manylinux/).
